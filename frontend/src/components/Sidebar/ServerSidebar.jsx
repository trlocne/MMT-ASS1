...
const ServerSidebar = () => {
  const { servers, currentServer, setCurrentServer, addServer } = useContext(GlobalContext);
  ...
  const handleAddServer = () => {
    if (serverName.trim()) {
      addServer(serverName, serverColor);
      setServerName('');
        ...
        <div className="w-16 bg-gray-900 flex flex-col items-center py-3 space-y-2 overflow-y-auto">
          ...
          {servers.map((server) => (
            <div
              ...
              style={{
                backgroundColor: currentServer === server.id ? server.color : '#4b5563', // Màu xám mặc định nếu không được chọn
              }}
            >
          ...
          {isModalOpen && (
            <div className="fixed m-0 top-0 inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50" style={{ margin: 0 }}>
              <div className="bg-gray-800 rounded-lg w-96 p-6">
                <h3 className="text-xl font-semibold mb-4 text-white">Create New Server</h3>
                <div className="mb-4">
                ...
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-400 mb-1">Server Color</label>
                  <input
                    type="color"
                    value={serverColor}
